...
class ParserManager {
    ...
    {
        return $this->getParserByUrl($url)->parseProduct();
    }

    public function parseCatalog($url, $max, $http_arg_id = null)
    {
        $urls = $this->getParserByUrl($url, $http_arg_id)->parseCatalog($max);
        if (!$urls)
            $urls = array();
    ...
    public function getParserByUrl($url, $http_arg_id = null)
    {
